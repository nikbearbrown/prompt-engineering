...
Persona patterns are the most overhyped prompt-engineering move and also one of the most misunderstood. The model will adopt a persona on request. What you get is rarely what you think you asked for.

Where the model genuinely helps: testing a persona prompt against itself by asking the model, out of persona, to evaluate what the persona changed about its responses. The cross-check exposes which traits actually transferred and which were cosmetic — usually fewer than the prompt author hoped.

Where the model does damage: producing personas that 'sound like' an expert without the underlying expertise. A persona is a register, not a knowledge base. A prompt that tells the model to respond as a senior cardiologist does not give the model cardiology it did not already have. It gives the response the rhythm of confidence — which is the most dangerous failure mode for a domain you cannot evaluate yourself.

The rule: a persona changes the texture of the answer. It does not change what the model knows.

---

## AI Wayback Machine

**Erving Goffman** was sociologist whose The Presentation of Self in Everyday Life (1959) built the framework for understanding personas and role performance.

**Run this:**

```
Who is Erving Goffman, and how does their work connect to persona prompting we covered in this chapter? Keep it to three paragraphs. End with the single most surprising thing about their career or ideas.
```

→ Search **"Erving Goffman"** on Wikipedia.

**Now make the prompt better.** Try one of these:

- Ask it to apply Erving Goffman's framework to a specific prompt-engineering decision.
- Add a constraint: "Answer including criticisms or limits of Erving Goffman's framework."
