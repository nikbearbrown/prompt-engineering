...
<!-- → [INFOGRAPHIC: A 2×2 grid with axes "Plausibility (high/low)" and "Truth (high/low)". Four quadrants filled with example claim types: high plausibility / high truth (capital of a major country), high plausibility / low truth (fabricated citation that sounds real), low plausibility / high truth (counterintuitive but correct fact), low plausibility / low truth (incoherent nonsense). The point is to make the orthogonality visual — all four quadrants are populated.] -->
...
<!-- → [DIAGRAM: Three-stage pipeline. Stage 1: model generates answer + Fact-Check List (inside the distribution — highlighted in one color). Stage 2: enumerated claims passed to external verifier — retrieval tool or human node (outside the distribution — different color). Stage 3: verifier returns "found / not found" for each claim, which feeds back as an Observation the model cannot override. Caption: the Fact-Check List is the handle; the external check is the governance.] -->
...
<!-- → [TABLE: Two-column table comparing Ungoverned Output vs. Governed Output for the three claim types — risk factor existence, verbatim quote, specific figure. Columns: Claim Type / What the model says / What external verification returns / Whether the error is catchable. Makes the governance value concrete and comparable.] -->
