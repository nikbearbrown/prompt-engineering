...
Chain a long reasoning trace and errors compound. If $P$ is the probability each step is correct and the steps were independent, the whole chain succeeds with probability roughly $P^N$. At $P = 0.95$ and $N = 10$, that is about 60 percent. At $N = 20$, about 36 percent. A wrong token early poisons the conditioning for every token after it. No prompt fixes this, because a prompt is just more tokens drawn from the same poisoned distribution.
...
<!-- → [DIAGRAM: The ReAct loop as a cycle. Three nodes labeled Thought, Action, Observation. Arrows: Thought → Action → Observation → Thought (repeat). On the Observation node, a separate arrow enters from outside labeled "External tool — not model-generated." Caption: Observation tokens enter from outside the model's distribution; this is what interrupts the P^N compounding chain.] -->
...
<!-- → [TABLE: Three-column comparison of structured output modes. Columns: Mode / What it guarantees / What it risks / When to use. Rows: Prompted JSON (no schema guarantee / trailing commas and stray prose / low-stakes informal), Constrained decoding answer-first (schema conformance / reasoning cost from early commitment / never — use reasoning-first instead), Constrained decoding reasoning-first (schema conformance + reasoning intact / none identified / production default).] -->
