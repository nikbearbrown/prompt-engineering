...
>
> **Sourcing note:** ReAct loop content condensed from the prior Ch. 10 (ReAct) draft — the $P^N$ derivation lives there in full; this chapter treats it as a given primitive and builds outward onto tool scaling, structured output, and multi-turn memory. Both previously-flagged figures verified on a dedicated fact-check pass: Tam et al. "Let Me Speak Freely?" is arXiv:2408.02442 (EMNLP 2024 Industry Track), and LongFuncEval (arXiv:2505.10570) reports a 7%–85% tool-count degradation band in its abstract.
...
**The counter-mechanism: format restriction can cost reasoning.** Here is the nuance that separates a careful engineer from a careless one. [Tam et al. (2024)](https://arxiv.org/abs/2408.02442) ("Let Me Speak Freely?", EMNLP 2024 Industry Track; arXiv:2408.02442, confirmed) found that forcing JSON/format restrictions *degrades* reasoning by roughly 10–15% on math and symbolic tasks versus free-form generation. Two distinct causes, and they matter for the fix:
...
One thing this chapter has held fixed is worth making explicit, because it changes which failures matter most. Every mechanism above assumes a human is somewhere in the loop — reading the onboarding agent's output, catching the hallucinated `send_welcome_packet` before a packet ships. The companion AI-fluency book, [Brown's *Botspeak*](#references), separates three operating modes by exactly this variable: **Augmentation** (a human is at the keyboard and every output crosses human eyes before use), **Automation** (the system runs on a schedule or trigger with no human present, so errors accumulate quietly across runs), and **Agency** (the system can take actions in the world before any human reviews them). The architectural rules here are necessary in all three, but they are not *sufficient* as you move toward Agency. *Botspeak*'s named failure mode for that mode is precise and worth importing: an agent takes *a locally reasonable but globally harmful action because it lacks context, stakeholder awareness, or proportionality.* A scoped tool set, a clean schema, and a summarize-and-restart memory policy keep the loop reliable; they do nothing about whether an irreversible action should have fired at all. That gap is filled by boundaries the loop itself cannot supply — permissions, escalation rules, and a human decision node on consequential or irreversible steps (the same commitment Ch. 4 made load-bearing for sycophancy). Reliability and authority-to-act are different axes; this chapter is about the first.
...
- Tam, Z. R., Wu, C.-K., Tsai, Y.-L., Lin, C.-Y., Lee, H.-y., & Chen, Y.-N. (2024). [Let Me Speak Freely? A Study on the Impact of Format Restrictions on Performance of Large Language Models](https://arxiv.org/abs/2408.02442). EMNLP 2024 (Industry Track), 2024.emnlp-industry.91. arXiv:2408.02442.
...
- Kate, K., Pedapati, T., Basu, K., Rizk, Y., Chenthamarakshan, V., Chaudhury, S., Agarwal, M., & Abdelaziz, I. (2025). [LongFuncEval: Measuring the effectiveness of long context models for function calling](https://arxiv.org/abs/2505.10570). arXiv:2505.10570. (Abstract reports 7%–85% degradation as tool count grows, 7%–91% as tool-response length grows.)
- Brown, N. B. *Botspeak: The Practitioner's Guide to AI Fluency.* Companion volume. *(Source of the three operating modes — Augmentation, Automation, Agency — and the Agency failure mode: a locally reasonable but globally harmful action.)*
